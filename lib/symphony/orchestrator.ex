defmodule Symphony.Orchestrator do
  ...
  def handle_call(:status, _from, state) do
    {:reply, OrchestratorStatus.status_payload(state), state}
  end

  def handle_call({:issue_status, issue_identifier}, _from, state) do
    {:reply, OrchestratorStatus.issue_status_payload(state, issue_identifier), state}
  end

  def handle_call(:current_config, _from, state) do
    {:reply, OrchestratorStatus.current_config_with_status_port(state), state}
  end
  ...
  def handle_call(:refresh, _from, state) do
    ...
    {:reply, {:ok, OrchestratorStatus.status_payload(state)}, state}
  end
  ...
  def handle_info({:agent_run_result, issue_id, result}, state) do
    case Map.get(state.running, issue_id) do
      ...
      entry ->
        recent_run = OrchestratorStatus.build_recent_run(entry, result)
        ...
        state =
          %{
            ...
            | completed: MapSet.put(state.completed, issue_id),
              ...
              recent_runs: OrchestratorStatus.prepend_recent_run(state.recent_runs, recent_run)
          }
    ...
    case Map.get(state.running, issue_id) do
      ...
      entry ->
        ...
        updated_entry =
          ...
          |> OrchestratorStatus.maybe_apply_runtime_update(payload)
    ...
    case Map.get(state.running, issue_id) do
      ...
      entry ->
        {updated_entry, details} =
          ...
          |> OrchestratorStatus.apply_phase_event(to_string(type), details)
  ...
  defp reconcile_review_handoffs(state) do
    pending =
      ...
      |> Enum.reduce(%{}, fn run, acc ->
        case OrchestratorStatus.pending_review_artifact(run) do
          nil -> acc
  ...
  defp pending_review_artifact_for_issue(state, issue_id) do
    ...
    |> Enum.find_value(fn run ->
      if run.issue_id == issue_id, do: OrchestratorStatus.pending_review_artifact(run), else: nil
    end)
  end
  ...
  defp do_merge_review_pr_for_done(state, run, artifact, issue) do
    case GitReview.merge_review_pr(run.workspace_path, artifact) do
      {:ok, merged_artifact} ->
        state =
          state
          |> OrchestratorStatus.update_recent_run_artifact(issue.id, merged_artifact)
        ...
        case Tracker.publish_review_handoff(state.config, issue, merged_artifact) do
          {:ok, republished_artifact} ->
            {:ok, OrchestratorStatus.update_recent_run_artifact(state, issue.id, republished_artifact)}
  ...
  defp summarize_issue(issue), do: OrchestratorStatus.summarize_issue(issue)
  defp summarize_routing(routing), do: OrchestratorStatus.summarize_routing(routing)
  ...
  defp maybe_log_runtime_event(event) do
    case OrchestratorRuntimeEvent.format(event) do
      nil -> :ok
